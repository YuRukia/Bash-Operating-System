...
HDDConfig=$(head -n1 HDD.cfg)
createConfig "sim.job"
simConfig=$(head -n1 sim.job)
...
do
        ...
        do
                ...
                1)
                        #Calls menu scripts
                        animationFunc
                        source ./menus/singleHDD.sh "$HDDConfig" "$simConfig"
                        #Increments times used
                        SHDD=$(($SHDD+1))
                        ;;
                2)
                        animationFunc
                        source ./menus/raid0.sh "$HDDConfig" "$simConfig"
                        R0=$(($RO+1))
                        ;;
                3)
                        animationFunc
                        source ./menus/raid1.sh "$HDDConfig" "$simConfig"
                        R1=$(($R1+1))
                        ;;
                4)
                        animationFunc
                        source ./menus/raid01.sh "$HDDConfig" "$simConfig"
                        R01=$(($R01+1))
                        ;;
                5)
                        animationFunc
                        source ./menus/raid100.sh "$HDDConfig" "$simConfig"
                        R100=$(($R100+1))
                        ;;
                6)
                        source ./admin.sh
